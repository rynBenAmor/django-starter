...
def send_confirmation_email(request, user):
    ...
    html_message = render_to_string('registration/emails/activation_email.html', {
        'user': user,
    ...
    try:    
        ...
        send_mail(subject,message=plain_message, html_message=html_message,  from_email=settings.DEFAULT_FROM_EMAIL, recipient_list=[user.email], fail_silently=False)
        #from_email=settings.DEFAULT_FROM_EMAIL,recipient_list=[settings.EMAIL_HOST_USER],  # consider Sending to yourself
    except Exception:
        pass
